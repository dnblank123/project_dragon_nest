...
DEFINE_LOG_CATEGORY_STATIC(MyLog, Display, All);
...
{
	UE_LOG(MyLog, Warning, L"%d", InValue);
}

void CLog::Log(float InValue)
{
	UE_LOG(MyLog, Warning, L"%f", InValue);
}

void CLog::Log(const FString& InValue)
{
	UE_LOG(MyLog, Warning, L"%s", *InValue);
}

void CLog::Log(const FVector& InValue)
{
	UE_LOG(MyLog, Warning, L"%s", *InValue.ToString());
}
